...
gam_wrap <- function(outcome, link = "log", data) {
  gam(get(outcome) ~ study_period + s(part_id, bs = "re"), data = data, 
      family = nb(link = link), weights = genderageweight_proportion)
...
pdt[part_age_group %in% c("60-69", "70+"), part_age_group := "60+"]
...
avg_conts_rr <- ggplot(m_periods[sample_type == "Adults"],
...
avg_conts_rr2 <- ggplot(m_periods[sample_type == "Adults"],
...
avg_conts_rr2_children <- ggplot(m_periods[sample_type == "Children"],
                                 aes(x = term, y = rr, color = mod)) +
  geom_hline(yintercept = 1, color = '#f79b57') +
  geom_point(shape = 15, size  = 1.5, position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = lci,
...
avg_conts_rr2_age <- ggplot(m_periods, aes(x = term, y = rr, color = mod)) +
  geom_hline(yintercept = 1, color = '#f79b57') +
  geom_point(shape = 15, size  = 1.5, position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = lci,
                    ymax = uci),
...
ggsave(avg_conts_rr2_age, filename = file.path("outputs", "cnt_rr_by_age_study_ages_weighted.png"),
       width = 9, height = 6)
