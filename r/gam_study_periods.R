...
mod_ref <- "study_period_sp_fs"

gam_wrap <- function(outcome, link = "log", data) {
  
  # gam(get(outcome) ~ study_period + s(part_id, by = study_period, bs = "re"), 
  # gam(get(outcome) ~ s(study_period, bs = "fs"),
  #     
      # gam(get(outcome) ~ s(study_period, bs = "fs") + s(part_id, bs = "re"),
      # gam(get(outcome) ~ study_period + s(part_id, bs = "re"),
     # gam(formula(get(outcome) ~  + study_period + s(study_period, bs = "fs")),
          # gam(get(outcome) ~  study_period,
              
      # gam(get(outcome) ~ study_period + s(part_id, bs = "re") + s(part_id, by = study_period, bs = "re"),
      data = data, 
