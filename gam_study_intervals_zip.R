...
# Functions ----------------------------
gam_wrap <- function(formula_, link = "identity", data) {
  gam(formula_,
      data = data, 
      weights = genderageweight_proportion,
      family = mgcv::ziP(link = link))
}
...
for (i in 1:length(formulas)) {
  ...
  for (i in 1:length(part_age_groups)){
    ...
    mod_subset <- gam_wrap(form, link = "identity", data = pdtage)
  ...
  # Save files
  plot_fname <- paste0("cnt_rr_mod_", mod_ref, "_zip.png")
  ggsave(avg_conts_rr2_facet, filename = file.path("outputs", plot_fname), 
         width = 9, height = 8)
  message(plot_fname)
  
  mod_fname <- paste0("mod_raw_", mod_ref, "_zip.qs")
  qs::qsave(mod_raw, file.path("data", mod_fname))
  message(mod_fname)
  
  cis_fname <- paste0("mod_cis_", mod_ref, "_zip.qs")
