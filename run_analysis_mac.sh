...
Rscript "dm101_calc_avg_contact_data.R" $boots

echo "Calculate mean contacts by age and risk group"
Rscript "dm102_calc_avg_contact_data_risk.R" $boots

echo "Calculate mean contacts by SE categories"
Rscript "dm102_calc_avg_contact_data_se.R" $boots

echo "Calculate proportion reported"
Rscript "dm111_calc_avg_proportions.R" $boots
